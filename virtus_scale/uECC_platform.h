...
#elif defined(NRF52_SERIES) && defined(UECC_USE_SDK_RNG)
// Disabled for the Arduino build — the sketch supplies its own RNG via
// uECC_set_rng() (SoftDevice hardware RNG), so the nRF5-SDK crypto RNG
// (app_error.h / nrf_crypto_rng.h) is not needed and not available here.
#include "app_error.h"
